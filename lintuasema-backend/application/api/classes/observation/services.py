...
def getDaySummary(day_id):
    stmt = text("SELECT " + prefix + "Observation.species AS species,"
                ...
                "(CASE WHEN " + prefix + "Observation.notes IS '' THEN null ELSE " + prefix + "Observation.notes END, ', ') " +
                ("WITHIN GROUP (ORDER BY " + prefix +
                 "Observation.notes)" if "sqlite" not in str(db.engine) else "") + "AS notes"
