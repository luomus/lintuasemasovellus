...
// Buckets:
  ...
  const constructTaysiHavainto = (startKello, endKello) => {
...
export function translateObservationParserError(t, message) {
  let msg;
  if (message.includes("unknownCharacter")) {
    msg = t("unknownCharacter", { char: (message.slice(-1)) });
  } else {
    msg = t(message);
  }
  return msg;
